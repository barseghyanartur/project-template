# Build:  docker build -t {{ project_name }}-test .
# Test:   docker run --rm {{ project_name }}-test
# Env:    docker run --rm {{ project_name }}-test -e py312
# Shell:  docker run --rm -it --entrypoint bash {{ project_name }}-test
