...
rebuild-docs:
	source $(VENV) && sphinx-apidoc . --full -o docs -H '{{ project_name }}' -A '{{ author_name }} <{{ author_email }}>' -f -d 20
...
clean-dev:
	...
	rm -rf dist/ src/{{ package_name }}.egg-info/ .cache/ .mypy_cache/ .ruff_cache/
...
update-version:
	...
	@if [ "$(UNAME_S)" = "Darwin" ]; then \
		...
		gsed -i 's/__version__ = "[0-9.]\+"/__version__ = "$(VERSION)"/' src/{{ package_name }}/__init__.py; \
		gsed -i 's/^:Version: [0-9.]\+/:Version: $(VERSION)/' ARCHITECTURE.rst; \
	else \
		sed -i 's/^version = "[0-9.]\+"/version = "$(VERSION)"/' pyproject.toml; \
		sed -i 's/__version__ = "[0-9.]\+"/__version__ = "$(VERSION)"/' src/{{ package_name }}/__init__.py; \
